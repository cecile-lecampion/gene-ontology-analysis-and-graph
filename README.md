...
Cliquer sur `Download R 4.0.5 for Windows` et suivre les instructions (en cas de nouvelle version le n° peuvent varier)
...
Cliquer sur `R-4.0.5.pkg` et suivre les instructions (en cas de nouvelle version le n° peuvent varier)
...
Le fichier a été installer par l'executable `TerminalUnixSetup.exe` :smile:
...
`output_curated_gene_ontology.tsv` est le fichier de sortie, celui que vous utiliserez pour faire la représentation graphique. Vous pouvez utiliser le nom actuelle ou définir un nom de votre choix.
